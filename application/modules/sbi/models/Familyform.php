 ...
 public function getagriculturedetails($id)
    {
        $select=$this->select()
                        ...
                        ->join(array('a'=>'ourbank_agriculture'),'e.id=a.land_id',array('a.id','a.landowner_name','a.villagename','a.survey_no','acre','gunta','value'))
 ...
 public function getagritotal($id)
  {
                      ...
                      ->setIntegrityCheck(false)
						->join(array('d'=>'ourbank_familymember'),array('d.id'))
                      	->where('d.familycode=?',$id)
						->join(array('e'=>'ourbank_family'),'d.family_id=e.id')
                      ->join(array('a'=>'ourbank_agriculture'),'e.id=a.land_id',array('a.id','value','Sum(value) as total'));
      // die($select->__toString($select));
      $result=$this->fetchAll($select);
      return $result->toArray();
  }
