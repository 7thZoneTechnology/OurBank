...
<?php
class Familysavings_Model_loandetails  extends Zend_Db_Table {
    protected $_name = 'ourbank_member';
        ...
        $select=$this->select()
                                ...
                                ->join(array('a'=>'ourbank_master_deposit'),array('a.id'));
        $result=$this->fetchAll($select);
        return $result->toArray();
//         die ($select->__toString($select));
        }

	public function getSavings($memberid)
        {
        $select=$this->select()
                                ->setIntegrityCheck(false)
...
//update the family details with respective to member id...
    ...
    $where[] = "id = '".$loanId."'";
    $db = $this->getAdapter();
    $result = $db->update('ourbank_savingdetails',$input,$where);
    }
